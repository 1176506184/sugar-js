<template>
  <div style="height: 100vh;margin: 0;padding: 0;border-right: 1px solid #ccc;border-left: 1px solid #ccc">
    <div class="layout_top" style="height: 20px;display: flex;align-items: center;justify-content: flex-start">
      <div>
        <span style="font-size: 15px;font-weight: bold;margin-right: 5px">pinterest博主-历史自动采集</span>
      </div>
    </div>
      ...
      <div v-if="isHaveBlogger">
        ...
        <div style="margin: 30px 0;">
          <div style="font-size: 16px">
            ...
            <span style="margin-left: 20px">已采集帖子：{{ collectNum }}</span>
            <span style="margin-left: 20px; color: green;">已入库成功：{{ successPostNum }}</span>
          </div>
...
async function dealTtHistory(Message) {
  ...
  if (Message.Message === 'updateActiveId') {
  ...
  } else if (Message.Message == "history" && Message.type == "pinterest" && Message.frameId.toString() === route.query.activeId.toString()) {
    // console.log(Message)
  ...
  } else if (Message.Message == "sendData" && Message.type == "pinterest" && Message.FrameId.toString() === route.query.activeId.toString()) {
    ...
    let num = postArray.length;
    console.log('采集数据数量:', num);
    console.log('采集的数据:', postArray);

    // 存数据接口
    let res = await hHttp("/BloggerCaptureHistoryNew/AddArticle", postArray);
    console.log('sendData res:', res);

    console.log(`传输状态：${res.state ? '成功' : '失败'}`);
    collectNum.value += num;
    if (res.state == true) {
      successPostNum.value += num;
    }

  } else if (Message.Message == "endToAlert" && Message.type == "pinterest" && Message.FrameId.toString() === route.query.activeId.toString()) {

    // 5s后钉钉发送采集完成提醒，保证数据传输完毕
