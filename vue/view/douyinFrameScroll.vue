...
setInterval(() => {
  ...
  chrome.tabs.query(
      ...
      function (tabs) {
        tabs.map((tab) => {
          if (!tab.url.includes('out.html#/DouyinFrameScroll') && tab.url.includes('douyin.com')) {
            uid.value += 1;
...
async function getNextCollect(u) {
  ...
  if (restartArr.length >= 3) {
    restartArr = [];
    active_page = null;
    await nextUrl('https://www.douyin.com/user/' + data.usereid);
  ...
  return new Promise((r, j) => {
    chrome.tabs.sendMessage(
        tab.id,
        {
          Message: "video_frame",
          Scroll: true,
          nextHref: 'https://www.douyin.com/user/' + data.usereid
        },
        function (response) {
